...
import { isCompletedOnTime } from '@/lib/date'
...
export default function HistoryPage() {
  ...
  const onTimeCount = (historyQuery.data ?? []).filter((entry) =>
    isCompletedOnTime(entry.completedAt, entry.deadline),
  ).length
  const productivityScore = totalCompleted > 0 ? Math.round((onTimeCount / totalCompleted) * 100) : 0

  const insightTitle =
    totalCompleted > 0 ? `이번 주 ${totalCompleted}개의 과업을 완료했습니다!` : '이번 주에는 아직 완료한 과업이 없습니다.'
