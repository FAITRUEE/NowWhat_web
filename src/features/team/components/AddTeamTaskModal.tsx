...
interface AddTeamTaskModalProps {
  open: boolean
  onClose: () => void
  onSubmit: (input: NewTeamTaskInput) => void
  members: TeamMember[]
}

export default function AddTeamTaskModal({ open, onClose, onSubmit, members }: AddTeamTaskModalProps) {
  ...
  useEffect(() => {
    ...
    setAssigneeUserId(members[0]?.userId ?? null)
  }, [open, members])

  if (!open) return null

  const canSubmit = title.trim() !== '' && deadline !== '' && assigneeUserId != null

  function handleSubmit(event: FormEvent) {
    event.preventDefault()
    if (!canSubmit || assigneeUserId == null) return
    onSubmit({ title, description, deadline, importance, estimatedMinutes, assigneeUserId })
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-primary/20 p-4 backdrop-blur-2xl">
      ...
      >
        <div className="flex items-center justify-between border-b border-surface-variant/30 p-8">
          <div>
            <h3 className="text-headline-md text-primary">업무 추가</h3>
            <p className="text-label-md text-on-surface-variant">담당자를 정해서 팀 보드에 바로 카드를 만드세요.</p>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="space-y-8 overflow-y-auto p-8">
          <div className="space-y-2">
            <label className="text-label-sm uppercase tracking-wider text-on-surface-variant">업무 제목</label>
            <input
              type="text"
              value={title}
          ...
          <div className="space-y-2">
            <label className="text-label-sm uppercase tracking-wider text-on-surface-variant">담당자</label>
            <select
              value={assigneeUserId ?? ''}
              onChange={(event) => setAssigneeUserId(Number(event.target.value))}
